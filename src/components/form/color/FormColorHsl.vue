<template>
	<div class="column spaced">
		...
		<FormRange v-model="l" :min="0" :max="100" :backg="lBackg">L</FormRange>
	</div>
</template>
...
<script setup lang="ts">
	...
	const h = computed({
		get: () => hsl.value.h,
		set(h) {
			hsl.value = Hsl.parse({h: Number(h), s: s.value, l: l.value});
		}
	});
	...
	const s = computed({
		get: () => hsl.value.s,
		set(s) {
			hsl.value = Hsl.parse({h: h.value, s: Number(s), l: l.value});
		}
	});
	...
	const l = computed({
		get: () => hsl.value.l,
		set(l) {
			hsl.value = Hsl.parse({h: h.value, s: s.value, l: Number(l)});
		}
	});
